...
public class PlutoniumBlock extends Block {
    ...
    public void goSupercritical(World world, BlockPos pos, Integer ticks) {
        ...
        QuantumAdds.LOGGER.error("SUPERCRITICAL");

        var closestPlayer = world.getClosestPlayer(pos.getX(), pos.getY(), pos.getZ(), 25, true);
        if (closestPlayer != null && !world.isClient()) {
            RadiationData.addRad((IEntityDataSaver) closestPlayer, QuantumRadiationTypes.ALPHA, random.nextInt(1500));
            RadiationData.addRad((IEntityDataSaver) closestPlayer, QuantumRadiationTypes.BETA, random.nextInt(1250));
            RadiationData.addRad((IEntityDataSaver) closestPlayer, QuantumRadiationTypes.GAMMA, random.nextInt(1000));
            RadiationData.addRad((IEntityDataSaver) closestPlayer, QuantumRadiationTypes.NEUTRON, random.nextInt(4000));
        }
    }
